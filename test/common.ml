...
struct
  module Index = struct
    include Index.Private.Make (Key) (Value) (IO) (Lock) (Mutex) (T)
  ...
  struct
    ...
    let empty_index ?(log_size = 4) ?flush_callback ?throttle () =
      ...
      let clone ?(fresh = false) ~readonly () =
        let t =
          Index.v ?flush_callback ?throttle ~cache ~fresh ~log_size ~readonly
            name
        in
    ...
    let with_full_index ?log_size ?flush_callback ?throttle ?size () f =
      call_then_close
        (full_index ?log_size ?flush_callback ?throttle ?size ())
        f
  end
