...
module Make (Platform : Common.Platform) = struct
  let ( // ) = Filename.concat
  let root = "_tests" // (Platform.name ^ ".main")
  ...
  module Semaphore = Platform.Semaphore
  ...
  module Live = struct
    let find_present_live ~io () =
      let* Context.{ rw; tbl; _ } = Context.with_full_index ~io () in
      Context.check_equivalence rw tbl

    let find_absent_live ~io () =
      let* Context.{ rw; tbl; _ } = Context.with_full_index ~io () in
      test_find_absent rw tbl

    let replace_live ~io () =
      let* Context.{ rw; _ } = Context.with_full_index ~io () in
      test_replace rw

    let different_size_for_key ~io () =
      let* Context.{ rw; _ } = Context.with_empty_index ~io () in
    ...
    let open_after_clear ~io () =
      let* Context.{ clone; rw; _ } = Context.with_full_index ~io () in
      Index.clear rw;
    ...
    let files_on_disk_after_clear ~io () =
      let root = Context.fresh_name "full_index" in
      let rw = Index.v ~io ~fresh:true ~log_size:Default.log_size root in
      ...
      Index.clear rw;
      Index.close rw;
      let module I = Platform.IO in
      let test_there path =
        match I.v_readonly ~io path with
  ...
  module DuplicateInstance = struct
    let find_present ~io () =
      let* Context.{ rw; tbl; clone; _ } = Context.with_full_index ~io () in
    ...
    let replace ~io () =
      let* Context.{ rw; clone; _ } = Context.with_full_index ~io ~size:5 () in
      let (_ : index) = clone ~readonly:false () in
      test_replace rw

    let membership ~io () =
      let* Context.{ tbl; clone; _ } = Context.with_full_index ~io () in
      let rw' = clone ~readonly:false () in
      check_equivalence_mem rw' tbl

    let fail_restart_fresh ~io () =
      let reuse_name = Context.fresh_name "empty_index" in
      let cache = Index.empty_cache () in
      let rw =
        Index.v ~io ~cache ~fresh:true ~readonly:false ~log_size:4 reuse_name
      in
      let exn = I.RO_not_allowed in
      Alcotest.check_raises "Index readonly cannot be fresh." exn (fun () ->
          let (_ : index) =
            Index.v ~io ~cache ~fresh:true ~readonly:true ~log_size:4 reuse_name
          in
          ());
      Index.close rw

    let sync ~io () =
      let* Context.{ rw; clone; _ } = Context.with_full_index ~io () in
    ...
    let duplicate_entries ~io () =
      let* Context.{ rw; clone; _ } = Context.with_empty_index ~io () in
  ...
  module Readonly = struct
    let readonly ~io () =
      let* Context.{ rw; clone; tbl; _ } = Context.with_empty_index ~io () in
      ...
      Index.flush rw;
      Index.sync ro;
      Context.check_equivalence ro tbl2

    let readonly_v_after_replace ~io () =
      let* Context.{ rw; clone; _ } = Context.with_full_index ~io () in
      let k = Key.v () in
      let v = Value.v () in
      Index.replace rw k v;
    ...
    let readonly_clear ~io () =
      let check_no_index_entry index k =
        Alcotest.check_raises (Fmt.str "Find %s key after clearing." k)
          Not_found (fun () -> ignore_value (Index.find index k))
      in
      let* Context.{ rw; tbl; clone; _ } =
        (* Ensure that the clear also wipes the LRU *)
        let lru_size = 10 in
        Context.with_full_index ~io ~lru_size ()
      in
      let ro = clone ~readonly:true () in
      Index.clear rw;
      Index.sync ro;
    ...
    let before l m =
      ...
      | `Before -> Semaphore.acquire "before" l
    ...
    let readonly_snapshot ~io () =
      ...
      Index.replace rw k v;
      let thread =
        Index.try_merge_aux ~force:true ~hook:(before merge sync) rw
      in
      Hashtbl.iter (Index.replace rw) tbl2;
      Index.flush rw;
      Context.check_equivalence rw tbl2;
      Context.check_equivalence ro tbl;
      Index.sync' ~hook:(after merge sync) ro;
      Context.check_equivalence ro tbl2;
      Semaphore.release merge;
      Index.await thread |> check_completed

    let fail_readonly_add ~io () =
      let* Context.{ clone; _ } = Context.with_empty_index ~io () in
      let ro = clone ~readonly:true () in
      let exn = I.RO_not_allowed in
    ...
    let readonly_add_log_before_clear ~io () =
      let* Context.{ rw; clone; _ } = Context.with_empty_index ~io () in
      let ro = clone ~readonly:true () in
      let k1, v1 = (Key.v (), Value.v ()) in
      Index.replace rw k1 v1;
    ...
    (** Readonly finds value in index before and after clear. Before sync the
        deleted value is still found. *)
    let readonly_add_index_before_clear ~io () =
      let* Context.{ rw; clone; _ } = Context.with_full_index ~io () in
      let ro = clone ~readonly:true () in
      Index.clear rw;
      let k1, v1 = (Key.v (), Value.v ()) in
    ...
    (** Readonly finds old value in log after clear and after new values are
        added, before a sync. *)
    let readonly_add_after_clear ~io () =
      let* Context.{ rw; clone; _ } = Context.with_empty_index ~io () in
      let ro = clone ~readonly:true () in
      let k1, v1 = (Key.v (), Value.v ()) in
      Index.replace rw k1 v1;
    ...
    (** Readonly finds old value in index after clear and after new values are
        added, before a sync. This is because the readonly instance still uses
        the old index file, before being replaced by the merge. *)
    let readonly_add_index_after_clear ~io () =
      let* Context.{ rw; clone; _ } = Context.with_empty_index ~io () in
      let ro = clone ~readonly:true () in
      Index.clear rw;
      let k1, v1 = (Key.v (), Value.v ()) in
    ...
    let readonly_open_after_clear ~io () =
      let* Context.{ clone; rw; _ } = Context.with_full_index ~io () in
      Index.clear rw;
    ...
    let readonly_sync_and_merge ~io () =
      let* Context.{ clone; rw; _ } = Context.with_empty_index ~io () in
      let ro = clone ~readonly:true () in
      ...
      Semaphore.acquire "replace" replace;
      Index.replace rw k2 v2;
    ...
    let readonly_sync_and_merge_clear ~io () =
      let* Context.{ clone; rw; _ } = Context.with_empty_index ~io () in
      let ro = clone ~readonly:true () in
      let merge = Semaphore.make false and sync = Semaphore.make false in
      let merge_hook =
    ...
    let reload_log_async ~io () =
      ...
      Alcotest.(check int)
        "reloadings of log async per merge" 1 !reload_log_async;
      Index.await t |> check_completed
    ...
    let tests ~io =
      List.map
        ...
        [
          ...
          ( "read old values in log after clear",
            `Quick,
            readonly_add_after_clear );
  ...
  module Close = struct
    ...
    let close_reopen_rw ~io () =
      let* Context.{ rw; tbl; clone; _ } = Context.with_full_index ~io () in
      Index.close rw;
      let w = clone ~readonly:false () in
      Context.check_equivalence w tbl

    let close_reopen_rw_more ~io () =
      let merge = Semaphore.make false in
      let merge_hook =
        I.Private.Hook.v @@ function
        | `After_clear ->
      ...
      in
      let* Context.{ rw; clone; tbl; _ } =
        Context.with_full_index ~io ~log_size:2 ()
      in
      let k1, v1 = (Key.v (), Value.v ()) in
      let k2, v2 = (Key.v (), Value.v ()) in
    ...
    let crash_and_continue ~io () =
      let merge = Semaphore.make false in
      let merge_hook =
        Log.debug (fun m -> m "crash_and_continue: merge_hook");
        I.Private.Hook.v @@ function
        | `After_clear ->
            Semaphore.release merge;
            raise Stop
    ...
    let open_readonly_close_rw ~io () =
      let* Context.{ rw; tbl; clone; _ } = Context.with_full_index ~io () in
      let ro = clone ~readonly:true () in
      Index.close rw;
      Context.check_equivalence ro tbl

    let close_reopen_readonly ~io () =
      let* Context.{ rw; tbl; clone; _ } = Context.with_full_index ~io () in
      Index.close rw;
      let ro = clone ~readonly:true () in
      Context.check_equivalence ro tbl

    let fail_api_after_close ~io () =
      let k = Key.v () in
      let v = Value.v () in
      let calls t =
        [
          ...
          ( "try_merge ~force:true",
            fun () ->
              ...
              | Ok `Aborted | Error _ ->
                  Alcotest.fail
                    "Unexpected return status from [try_merge ~force:true] \
                     after close" );
    ...
    let aborted_merge ~io () =
      let* Context.{ rw; _ } =
        Context.with_full_index ~io ~throttle:`Block_writes ~size:100 ()
      in
      ...
      let hook = function
        ...
        | `After_first_entry ->
            Semaphore.acquire "abort_signalled" abort_signalled
        | `After_clear | `After ->
            Alcotest.failf
      ...
      Semaphore.acquire "close_request" close_request;
      ...
      | Ok `Aborted -> (
          ...
          | `Skip err ->
              Log.warn (fun m -> m "`aborted_fd` was skipped: %s" err))
  ...
  module Filter = struct
    ...
    (** Test that all bindings are kept when using [filter] with a true
        predicate. *)
    let filter_none ~io () =
      let* Context.{ rw; tbl; _ } = Context.with_full_index ~io ~lru_size () in
    ...
    (** Test that all bindings are removed when using [filter] with a false
        predicate. *)
    let filter_all ~io () =
      let* Context.{ rw; _ } = Context.with_full_index ~io ~lru_size () in
      Index.filter rw (fun _ -> false);
      Context.check_equivalence rw (Hashtbl.create 0)
    ...
    let clone_then_filter ~io () =
      ...
      Index.filter rw (fun (k', _) -> not (String.equal k k'));
      Context.check_equivalence rw tbl;
      Context.check_equivalence rw2 tbl

    (** Test that the results of [filter] are propagated to a clone which was
        created after. *)
    let filter_then_clone ~io () =
      let* Context.{ rw; tbl; clone; _ } =
        Context.with_full_index ~io ~lru_size ()
      in
      let k = random_existing_key tbl in
    ...
    (** Test that using [filter] doesn't affect fresh clones created later at
        the same path. *)
    let empty_after_filter_and_fresh ~io () =
      let* Context.{ rw; tbl; clone; _ } =
        Context.with_full_index ~io ~lru_size ()
      in
      let k = random_existing_key tbl in
      ...
      Context.check_equivalence rw2 (Hashtbl.create 0)
  ...
  module Throttle = struct
    let add_binding ?hook t =
      ...
      | binding, Some _ ->
          Alcotest.failf
            "New binding %a triggered an unexpected merge operation" pp_binding
            binding
    ...
    let merge ~io () =
      ...
      let hook =
        Hook.v @@ function `Before -> Semaphore.acquire "m" m | _ -> ()
      in
    ...
    let implicit_merge ~io () =
      let log_size = 4 in
      let* Context.{ rw; _ } =
        Context.with_empty_index ~io ~log_size ~throttle:`Overcommit_memory ()
      in
      let m = Semaphore.make false in
      let hook =
        Hook.v @@ function `Merge `Before -> Semaphore.acquire "m" m | _ -> ()
      in
    ...
    let tests ~io =
      List.map
        (fun (name, mode, test) -> (name, mode, test ~io))
        [
          ("force merge", `Quick, merge);
          ("implicit merge", `Quick, implicit_merge);
        ]
  end

  module Io_array = Io_array.Make (Platform)
  module Force_merge = Force_merge.Make (Platform)
  module Test_lru = Test_lru.Make (Platform)
  module Flush_callback = Flush_callback.Make (Platform)

  let tests ~io =
    Common.report ();
    List.map
      (fun (name, test) -> (name, test ~io))
      [
