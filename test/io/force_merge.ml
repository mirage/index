...
module Make (Platform : Common.Platform) = struct
  let root = Filename.concat "_tests" (Platform.name ^ ".force_merge")
  ...
  *)
  ...
  let find_in_async_generation_change ~io () =
    let* { Context.rw; clone; _ } = Context.with_full_index ~io () in
    let w = rw in
    let r1 = clone ~readonly:true () in
    let k1 = Key.v () in
  ...
  let find_in_async_same_generation ~io () =
    let* { Context.rw; clone; _ } = Context.with_full_index ~io () in
    let w = rw in
    let r1 = clone ~readonly:true () in
    let k1 = Key.v () in
  ...
  let sync_before_and_after_clearing_async ~io () =
    let* { Context.rw; clone; _ } = Context.with_full_index ~io () in
    let w = rw in
  ...
  (** RW adds a value in log and flushes it, so every subsequent RO sync should
      find that value. But if the RO sync occurs during a merge, after a clear
      but before a generation change, then the value is missed. Also test ro
      find at this point. *)
  let sync_after_clear_log ~io () =
  ...
  let test_non_blocking_clear ~io () =
    let* Context.{ rw; _ } = Context.with_empty_index ~io () in
    ...
    Semaphore.acquire "merge_started" merge_started;
    add_bindings rw;
  ...
  (** The test consists of aborting a first merge after one entry is added in
      the ./merge file and checking that a second merge succeeds. Regression
      test for PR 211 in which the second merge was triggering an assert
      failure. *)
  let test_abort_merge ~io ~abort_merge () =
    let* { Context.rw; clone; _ } = Context.with_full_index ~io () in
    let merge_started = Semaphore.make false and merge = Semaphore.make false in
    let merge_hook =
      Hook.v @@ function
