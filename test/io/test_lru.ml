...
module Make (Platform : Common.Platform) = struct
  module Context =
    Common.Make_context
      ...
      (struct
        let root =
          Filename.concat "_tests" (Platform.name ^ ".test_log_with_lru")
      end)
  ...
  let test_replace_and_find ~io () =
    let lru_size = 1 in
    let* { rw = idx; _ } = Context.with_empty_index ~io ~lru_size () in
  ...
  let test_clear ~io () =
    let lru_size = 1 in
    let* { rw = idx; _ } = Context.with_full_index ~io ~lru_size () in
