...
module Make (Platform : Common.Platform) = struct
  module Context =
    Common.Make_context
      (Platform)
      (struct
        let root = Filename.concat "_tests" (Platform.name ^ ".flush_callback")
      end)
  ...
  module Mutable_callback = struct
    type t = {
      ...
      require_callback :
        'a.
        ?at_least_once:unit -> ?callback:(unit -> unit) -> (unit -> 'a) -> 'a;
  ...
  let test_close ~io () =
    ...
    in
    Context.with_empty_index ~io ~flush_callback:(fail "empty index") ()
      Context.ignore;

    Context.with_full_index ~io ~flush_callback:(fail "fresh index") ()
      Context.ignore;

    let calls = ref 0 in
    Context.with_empty_index ~io
  ...
  let test_flush ~io () =
    ...
    let* Context.{ rw; clone; _ } =
      Context.with_empty_index ~io ~flush_callback ()
    in
    ...
    require_callback
      ~callback:(fun () ->
        Log.app (fun m ->
            m "Checking that newly-added binding %a is not yet visible"
              pp_binding binding);
        Index.sync ro;
  ...
  let test_replace ~io () =
    ...
    in
    let* Context.{ rw; clone; _ } =
      Context.with_empty_index ~io ~log_size ~flush_callback ()
    in
    let ro = clone ~readonly:true () in
    ...
    let overflow_binding, merge_promise =
      require_callback
        ~callback:(fun () ->
          Log.app (fun m ->
              m
                "Checking newly-added bindings are not visible from a synced \
                 RO instance until [flush_callback] is called");
          Index.sync ro;
          Context.check_disjoint ro bindings)
        (fun () ->
          Index.replace_random
            ~hook:
              (I.Private.Hook.v (function
                | `Merge `Before -> Semaphore.acquire "do_merge" do_merge
                | _ -> ()))
            rw)
    in
    ...
    Index.sync ro;
    Context.check_equivalence ro bindings;
