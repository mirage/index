...
module Make
    (IO : Index.Platform.IO)
    (Lock : Index.Platform.Lock)
    (Mutex : Index.MUTEX)
    (T : Index.Thread.S) =
struct
  open Common.Make (IO) (Lock) (Mutex) (T)
  ...
  let test_replace t =
    let k = Key.v () and v = Value.v () and v' = Value.v () in
    Index.replace t k v;
    Index.replace t k v';
  ...
  module Readonly = struct
    ...
    let readonly_clear () =
      let check_no_index_entry index k =
        Alcotest.check_raises (Fmt.strf "Find %s key after clearing." k)
      ...
      Index.close'
        ~hook:
          (I.Private.Hook.v (fun `Abort_signalled ->
               Mutex.unlock abort_signalled))
        rw;
      Fmt.pr "Parent: awaiting merge result\n%!";
      Index.await merge_promise |> function
      | Ok `Completed ->
          Alcotest.fail
            "Force_merge returned `Completed despite concurrent close"
      | Error (`Async_exn exn) ->
          Alcotest.failf
            "Asynchronous exception occurred during force_merge: %s"
  ...
  module Throttle = struct
    ...
    let implicit_merge () =
      ...
      let m = locked_mutex () in
      let hook =
        Hook.v @@ function `Merge `Before -> Mutex.lock m | _ -> ()
      in
